...
class DemoLoginView(APIView):
    """Issue a token for the appliance's pre-provisioned demo viewer.

    The endpoint is intentionally unavailable outside the dedicated demo
    container.  The viewer credentials remain in that container's .env and
    are never sent to, or embedded in, the browser bundle.
    """

    permission_classes = [AllowAny]

    def post(self, request):
        if not settings.DEMO_AUTO_LOGIN_ENABLED or not settings.DEMO_AUTO_LOGIN_USERNAME:
        ...
        try:
            user = (
                User.objects.select_related('profile')
                .prefetch_related('profile__security_groups')
                .get(username__iexact=settings.DEMO_AUTO_LOGIN_USERNAME)
            )
        except User.DoesNotExist:
            return Response({'detail': 'Демонстрационная учётная запись недоступна.'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)

        profile = getattr(user, 'profile', None)
        if not user.is_active or not profile or profile.status != UserStatus.ACTIVE:
            return Response({'detail': 'Демонстрационная учётная запись недоступна.'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)

        log_auth_event(request, 'login_success', user=user, details='demo_auto_login')
        return Response({
