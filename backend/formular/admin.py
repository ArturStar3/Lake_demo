...
class DemoScenarioAdmin(ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Автозапуск демонстрации', {
            'description': (
                'Отметьте сценарий, который должен запускаться автоматически '
                'при открытии демонстрационной версии.'
            ),
            'fields': ('is_default', 'loop', 'auto_advance', 'default_step_duration_ms'),
        }),
        ('Содержание сценария', {
            'fields': ('sequence', 'mosaic', 'tableau'),
        }),
    )
    ...
    @admin.action(description='Назначить сценарием, запускаемым по умолчанию')
    def set_as_default(self, request, queryset):
        if queryset.count() != 1:
            self.message_user(
                request,
                'Выберите ровно один сценарий для автозапуска.',
                level=messages.ERROR,
            )
            return

        scenario = queryset.get()
        scenario.is_default = True
        scenario.save(update_fields=['is_default'])
        self.message_user(
            request,
            f'Сценарий «{scenario.title}» будет запускаться по умолчанию.',
